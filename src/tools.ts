 ...
 */
export function resolveToolDescriptor(name: string | undefined): ToolDescriptor | undefined {
  if (name === undefined || name === '') return undefined
  const normalized = name.trim().toLowerCase().replace(/-/g, '_')
  for (const descriptor of TOOL_DESCRIPTORS) {
    for (const alias of descriptor.aliases) {
      if (normalized === alias || normalized.startsWith(`${alias}_`)) return descriptor
    }
  }
  return undefined
}
