...
function openInBrowser(url: string): boolean {
  try {
    // Windows: the URL MUST stay quoted all the way to cmd - an unquoted `&`
    // (every pairing URL has query params) is a command separator to cmd and
    // truncates the URL. Pre-quote it and pass windowsVerbatimArguments so
    // Node does not strip or re-escape the quotes.
    const command =
      process.platform === 'win32'
        ? { cmd: 'cmd', args: ['/d', '/s', '/c', 'start', '""', `"${url}"`] }
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const acquireBridgeLock = (): boolean => {
    ...
    try {
      ...
      // A lock held by OUR OWN pid is a leftover from a previous apply of
      // this plugin in the same process (config reload / plugin update):
      // that run's ctx.effect cleanup already disposed its bridge, so the
      // lock is safe to take over. Without this branch a reload could never
      // restart the bridge until the whole process exited.
  ...
  /** Release the bridge lock when (and only when) this process owns it. */
  const releaseBridgeLock = (): void => {
    try {
      const lockPath = join(dataDir, 'bridge.lock')
  ...
  // A crash leaves the file for the stale-takeover path; a normal exit cleans up.
  ...
  const startBridge = (credentials: StoredCredentials): void => {
    // Dispose the previous bridge FIRST: the lock is per-process, and the
    // old instance must release its connection before the new one starts.
    if (active !== undefined) {
