...
export function redactInlineSecrets(value: string): string {
  const redactAssign = (match: string, lead: string, key: string): string =>
    SENSITIVE_NAME.test(key) ? `${lead}${key}=[redacted]` : match
  const redactFlag = (match: string, lead: string, flag: string, sep: string): string =>
    SENSITIVE_NAME.test(flag.replace(/^-+/, '')) ? `${lead}${flag}${sep}[redacted]` : match
  return value
...
export function basenameOnly(text: string): string {
  if (text === '') return text
...
export function redactPaths(text: string): string {
  return text.replace(PATH_LIKE, (match, lead: string, path: string) => `${lead}${basenameOnly(path)}`)
}
 ...
 */
export function sanitizeToolDetail(text: string, sanitizer: string | undefined): string {
  if (text === '' || sanitizer === undefined) return text
