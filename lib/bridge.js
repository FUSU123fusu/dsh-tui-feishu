...
export class Bridge {
        ...
        this.turnDisposers.push(subscribe((sessionId, event) => {
            ...
            if (chatId === undefined) {
                ...
                if (!this.foreignSessions.has(sessionId)) {
                    if (this.foreignSessions.size > 256)
     ...
     */
    async deliverBatch(chatId, items) {
        const first = items[0];
        if (first === undefined)
            return;
        if (items.length === 1) {
            if (first.kind === 'text')
                return this.deliver(chatId, first.text);
     ...
     */
    async resolveImageBlocks(chatId, messageId, imageKey, caption) {
     ...
     */
    async openLateTurn(chatId, sessionId) {
        const queue = this.queuedTurns.get(chatId);
        const queuedTitle = queue !== undefined && queue.length > 0 ? queue.shift() : undefined;
        if (queue !== undefined && queue.length === 0)
            this.queuedTurns.delete(chatId);
        const state = { title: queuedTitle ?? '⏰ Agent', content: '', rows: [], openThink: false, expanded: false, sessionId, startedAt: Date.now(), toolStarts: new Map() };
    ...
    async handleSessionEvent(sessionId, event) {
        ...
        switch (event.type) {
            ...
            case 'assistant/chunk': {
                if (state === undefined)
                    state = await this.openLateTurn(chatId, sessionId);
            ...
            case 'tool/result': {
                ...
                if (matched >= 0) {
                    ...
                    if (row !== undefined && row.kind === 'tool') {
                        ...
                        const resultText = descriptor?.noResult === true && !error
                            ? ''
                            : sanitizer === undefined
