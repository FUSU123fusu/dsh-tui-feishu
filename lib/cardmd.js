...
const REASONING_BLOCK = new RegExp(`<\\s*${REASONING_TAG}\\s*>[\\s\\S]*?<\\s*/\\s*${REASONING_TAG}\\s*>`, 'gi');
...
export function stripReasoningTags(text) {
    ...
    if (result.trimStart().startsWith('Reasoning:'))
        return '';
    return result;
}
...
function findTablesOutsideCodeBlocks(text) {
    const codeRanges = [];
    ...
    for (const match of text.matchAll(tableRe)) {
        if (match.index === undefined || inCode(match.index))
            continue;
 ...
 */
export function downgradeTables(text, limit = MAX_CARD_TABLES) {
...
export function stripInvalidImageKeys(text) {
    if (!text.includes('!['))
        return text;
    return text.replace(/!\[([^\]]*)\]\(([^)\s]+)\)/g, (match, _alt, url) => url.startsWith('img_') ? match : '');
}
/**
 ...
 */
export function optimizeMarkdown(text) {
    try {
        const mark = '___CB_';
        const codeBlocks = [];
 ...
 */
export function splitLongText(text, limit = MAX_CHUNK_CHARS) {
...
export function formatCodeBlock(content, language = 'text') {
...
export function prettyJsonOrText(value) {
